...
export default function Landing({ openFornecedor, openMedico, onLoginSuccess }) {
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      if (e.key === "Escape" && !loading) setShowComum(false);
    };
  ...
  const handleModalLogin = async () => {
    ...
    try {
      ...
      const data = await apiService.login(loginEmail, loginSenha);

      if (onLoginSuccess) {
        onLoginSuccess(data.user);
      } else {
        alert(`Login bem-sucedido! Bem-vindo(a), ${data.user.name || "Usuário"}.`);
      }
    ...
    <>
      ...
      {showComum && (
        <div
          ...
          onClick={(e) => e.target === e.currentTarget && !loading && setShowComum(false)}
        >
          <div className="modal">
            <button
              className="modal-x"
              onClick={() => !loading && setShowComum(false)}
              aria-label="Fechar"
              disabled={loading}
            >
              ✕
            ...
            <div className="fg">
              ...
              <input 
                type="email" 
                placeholder="seu-email@medilink.com.br" 
                value={loginEmail}
            ...
            <div className="fg">
              ...
              <input 
                ...
                onKeyDown={(e) => e.key === "Enter" && !loading && handleModalLogin()}
                disabled={loading}
              />
            </div>

            {loginError && (
              <div style={{ color: "#e74c3c", fontSize: ".85rem", marginBottom: "1rem", fontWeight: "500" }}>
                ⚠️ {loginError}
              </div>
            )}

            <button 
              className="modal-btn m-green" 
