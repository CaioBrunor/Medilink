...
export default function MedCheckoutModal({ product: p, onConfirm, onClose }) {
  ...
  const precoUnidade = p.price || p.preco || 0;
  const nomeProduto = p.name || p.nome;
  const fornecedorNome = p.fornecedor || "Fornecedor Parceiro";

  const sub = (precoUnidade * qty).toFixed(2);
  const com = (precoUnidade * qty * 0.05).toFixed(2);
  const tot = (precoUnidade * qty * 1.05).toFixed(2);
      ...
      <div className="f-modal-box">
        ...
        <div className="med-checkout">
          <div className="med-co-product">
            ...
            <div>
              <h4 style={{ fontFamily: "'Syne',sans-serif", fontSize: '1rem' }}>{nomeProduto}</h4>
              <p style={{ color: 'var(--f-text2)', fontSize: '.82rem' }}>{fornecedorNome} {p.unidade ? `· ${p.unidade}` : ''}</p>
            </div>
          </div>
